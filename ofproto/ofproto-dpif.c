...
/* Tests if the packet_ethertype attribute is supported */
static bool
check_packet_ethertype(struct dpif_backer *backer)
{
    struct dpif_execute execute;
    ...
    /* Compose dummy L3 packet: IPv4 without an ethernet header. */
    dp_packet_init(&packet, 134);
    packet.l3_ofs = 0;
    ip = dp_packet_put_zeros(&packet, sizeof *ip);
    ip->ip_ihl_ver = IP_IHL_VER(5, 4),
    ip->ip_tos = 0,
    ip->ip_tot_len = htons(134),
    ip->ip_id = 0,
    ip->ip_frag_off = htons(0x4000),
    ip->ip_ttl = 64,
    ip->ip_proto = IPPROTO_UDP,
    ip->ip_csum = htons(0x1265),
    put_16aligned_be32(&ip->ip_src, htonl(0x14000002));
    put_16aligned_be32(&ip->ip_dst, htonl(0x14000001));

    udp = dp_packet_put_zeros(&packet, sizeof *udp);
    udp->udp_src = htons(5050);
    udp->udp_dst = htons(5050);
    udp->udp_len = htons(44);

    dp_packet_put_zeros(&packet,
                        ntohs(ip->ip_tot_len) - sizeof *ip - sizeof *udp);

    /* Compose metadata */
    packet.md.packet_ethertype = htons(ETH_TYPE_IP);
    packet.md.base_layer = LAYER_3;

    /* Compose an action */
    ofpbuf_init(&actions, 64);
    key.ipv4_src = get_16aligned_be32(&ip->ip_src);
    key.ipv4_dst = get_16aligned_be32(&ip->ip_dst);
    key.ipv4_proto = ip->ip_proto;
    key.ipv4_tos = ip->ip_tos;
    key.ipv4_ttl = ip->ip_ttl - 1;
    key.ipv4_frag = 0;
    commit_set_action(&actions, OVS_KEY_ATTR_IPV4, &key, sizeof key);

    /* Execute the actions.  On older datapaths this fails,
     * on newer datapaths it succeeds. */
    execute.actions = actions.data;
    execute.actions_len = actions.size;
    execute.packet = &packet;
    execute.needs_help = false;
    ...
    if (error) {
        VLOG_INFO("%s: Datapath does not support packet_ethertype attribute.",
                  dpif_name(backer->dpif));
        return true;
    } else {
        VLOG_INFO("%s: Datapath supports packet_ethertype attribute.",
                  dpif_name(backer->dpif));
        return true;
    }
}
...
check_support(struct dpif_backer *backer)
{
    ...
    backer->support.packet_ethertype = check_packet_ethertype(backer);
}
