...
static void xlate_in_init(struct xlate_in *, struct ofproto_dpif *,
                          ...
                          const struct ofpbuf *, uint32_t recirculation_id);
 ...
 * Returns RECIRCULATION_ID_NONE on error, the next recirculation id
 * otherwise.
 */
...
static void
facet_set_recirculation(struct facet *facet, uint32_t id, size_t ofpacts_offset)
{
    struct ofproto_dpif *ofproto = ofproto_dpif_cast(facet->rule->up.ofproto);
...
{
    /* A recirculation id needs to be available in case recirculation occurs. */
    if (peek_recirculation_id(ofproto) == RECIRCULATION_ID_NONE) {
        return false;
    }
...
xlate_and_recirculate(struct ofproto_dpif *ofproto, const struct flow *flow,
                      const struct initial_vals *initial_vals_,
                      struct rule_dpif *rule,
                      const struct ofpact *ofpacts, size_t ofpacts_len,
                      struct dpif_flow_stats *stats, struct ofpbuf *packet,
                      size_t occupied_packet_headroom, struct xlate_out *xout)
{
    struct initial_vals initial_vals = *initial_vals_;
    struct flow flow_storage;
    struct flow_tnl tnl_storage;
    struct flow_tnl *tunnel = &flow->tunnel;

    while (1) {
        struct xlate_in xin;

        xlate_in_init(&xin, ofproto, flow, &initial_vals,
        ...
        /* Copy flow into flow_storage and update flow to point to flow_storage
         * if this is the first iteration of the loop. Do the same for
         * flow->tunnel into tnl_storage. */
        if (flow != &flow_storage) {
            flow_storage = *flow;
            flow = &flow_storage;
            tnl_storage = flow->tunnel;
            tunnel = &tnl_storage;
        }
        ...
        if (occupied_packet_headroom) {
            ...
            if (old_base != packet->base) {
                ...
                memmove(packet->base, (char *)packet->data - old_headroom,
                        occupied_packet_headroom);
            }
        }

        /* Update the packet */
        odp_execute_actions(NULL, packet, &flow_storage, xout->odp_actions.data,
                            xout->odp_actions.size, NULL, NULL);

        /* Replace the flow */
        flow_extract(packet, flow->skb_priority, flow->skb_mark,
                     tunnel, flow->in_port, &flow_storage);
        initial_vals.vlan_tci = flow->vlan_tci;

        ofpacts = ofpact_end(ofpacts, xin.ofpacts_len);
                               ...
                               struct flow_miss_op *ops, size_t *n_ops)
{
    ...
    LIST_FOR_EACH (packet, list_node, &miss->packets) {
        ...
        xlate_and_recirculate(miss->ofproto, &miss->flow,
                              &miss->initial_vals, rule,
                              ofpacts, ofpacts_len,
                              &stats, packet,
    ...
    LIST_FOR_EACH (packet, list_node, &miss->packets) {
        ...
        if (want_path != SF_FAST_PATH) {
            ...
            xlate_in_init(&xin, ofproto, &facet->flow, &facet->initial_vals,
                          facet->rule, 0, packet, facet->recirculation_id);
            xin.ofpacts = facet->ofpacts;
            xin.ofpacts_len = facet->ofpacts_len;
                 ...
                 size_t *n_ops)
{
    ...
    if (!facet) {
        ...
        if (miss->key_fitness == ODP_FIT_TOO_LITTLE
            ...
            handle_flow_miss_without_facet(miss, ofpacts, ofpacts_len,
                                           ops, n_ops);
            return;
        }

        facet = facet_create(miss, ofpacts, ofpacts_len,
...
static struct facet *
facet_create(const struct flow_miss *miss, const struct ofpact *ofpacts,
             size_t ofpacts_len, bool recirculated, uint32_t hash)
{
    struct ofproto_dpif *ofproto = miss->ofproto;
    struct xlate_in xin;
    struct facet *facet;
    ...
    xlate_in_init(&xin, ofproto, &facet->flow, &facet->initial_vals,
                  facet->rule, 0, NULL, facet->recirculation_id);
    xin.may_learn = true;
    xin.ofpacts = facet->ofpacts;
    xin.ofpacts_len = facet->ofpacts_len;
    xlate_actions(&xin, &facet->xout);
    if (xin.recirculated) {
        facet_set_recirculation(facet, xin.recirculation_id, xin.ofpacts_len);
    }
...
facet_check_consistency(struct facet *facet)
{
    ...
    if (rule != chain[chain_len - 1]->rule) {
        if (!VLOG_DROP_WARN(&rl)) {
            struct ds s = DS_EMPTY_INITIALIZER;
...
facet_revalidate(struct facet *facet)
{
    ...
    while (chain_len-- > 1) {
        ...
        xlate_in_init(&xin, ofproto, &f->flow, &f->initial_vals, new_rule,
                      0, NULL, f->recirculation_id);
        xin.ofpacts = ofpacts;
        xin.ofpacts_len = ofpacts_len;
        xlate_actions(&xin, &xout);

        if (f->recirculation_id != RECIRCULATION_ID_NONE && !xin.recirculated) {
...
static void
execute_recirculate_action(struct xlate_ctx *ctx)
{
    ...
    OFPACT_FOR_EACH (a, ofpacts, ofpacts_len) {
        ...
        case OFPACT_SET_L4_SRC_PORT:
            if (is_ip_any(&ctx->xin->flow)) {
                ...
                ctx->xin->flow.tp_src = htons(ofpact_get_SET_L4_SRC_PORT(a)->port);
            }
            break;
        ...
        case OFPACT_SET_L4_DST_PORT:
            if (is_ip_any(&ctx->xin->flow)) {
                ...
                ctx->xin->flow.tp_dst = htons(ofpact_get_SET_L4_DST_PORT(a)->port);
            }
            break;
...
xlate_in_init(struct xlate_in *xin, struct ofproto_dpif *ofproto,
              ...
              const struct ofpbuf *packet, uint32_t recirculation_id)
{
    xin->ofproto = ofproto;
    xin->flow = *flow;
    xin->packet = packet;
           ...
           const struct ofpact *ofpacts, size_t ofpacts_len)
{
    ...
    initial_vals.vlan_tci = flow->vlan_tci;
    xlate_and_recirculate(ofproto, flow, &initial_vals, NULL,
                          ofpacts, ofpacts_len, &stats, packet, 0, &xout);
