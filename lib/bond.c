                         ...
                         const uint8_t eth_dst[ETH_ADDR_LEN])
{
     ...
     * drop all incoming traffic except if lacp_fallback_ab is true. */
                    ...
                    struct flow_wildcards *wc, uint16_t vlan)
{
    ...
    balance = (bond->lacp_status == LACP_CONFIGURED)?
                ((bond->lacp_fallback_ab && 
                  (bond->balance==BM_AB || bond->balance==BM_TCP))?BM_AB:-1):
                bond->balance;

    switch (balance) {
    case -1:
        /* LACP has been configured on this bond but negotiations were
         * unsuccussful and fallback not configured.  Drop all traffic. */
        return NULL;
