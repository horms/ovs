    ...
    OVS_NO_THREAD_SAFETY_ANALYSIS
{
    ...
    bool steal = may_steal;

    if (may_steal) {
        /* Do not allow stealing packet if we are in the middle
         * of recirculation, or we are about to start recricualtion. */
        if ((*depth) || (type == OVS_ACTION_ATTR_RECIRC)) {
            steal = false;
        }
    }
