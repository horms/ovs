				  ...
				  struct eth_types *eth_types)
{
	...
	nla_for_each_nested(a, attr, rem) {
		...
		case OVS_ACTION_ATTR_PUSH_MPLS: {
			 ...
			 * of ethertypes that have a known tag order.
			 */
			for (i = eth_types->start; i < eth_types->end; i++)
				if (eth_types->types[i] != htons(ETH_P_IP) &&
				    eth_types->types[i] != htons(ETH_P_IPV6) &&
				    eth_types->types[i] != htons(ETH_P_ARP) &&
				    eth_types->types[i] != htons(ETH_P_RARP) &&
				    !eth_p_mpls(eth_types->types[i]))
					return -EINVAL;
